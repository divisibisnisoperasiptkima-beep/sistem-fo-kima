...
export default function IspPortalPage({ session, page }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
      ...
      setError(err.message || "Gagal memuat portal layanan pelanggan.");
    } finally {
      setLoading(false);
    }
  ...
  const pageCopy = {
    "isp-ringkasan": { eyebrow: "Ruang kerja mitra", title: "Ringkasan Penugasan ISP", description: "Gambaran pelanggan, lokasi, dan layanan yang ditugaskan KIMA kepada ISP Anda." },
    "isp-kontrak": { eyebrow: "Penugasan layanan", title: "Kontrak & Lokasi", description: "Informasi kontrak dan titik sambungan fiber optic yang telah ditetapkan KIMA." },
    "isp-dokumen": { eyebrow: "Pusat berkas", title: "Dokumen Penugasan", description: "Buka atau unggah dokumen yang terkait dengan kontrak dan penugasan ISP." },
  }[page] || { eyebrow: "Portal ISP", title: "Portal Mitra ISP", description: "Data layanan yang ditetapkan KIMA untuk ISP Anda." };
  ...
  if (loading) return <div className="p-8 text-sm text-white/60">Memuat data layanan…</div>;
    ...
    <div className="flex flex-col gap-5 pb-8">
      ...
      {page === "isp-ringkasan" && (
        <section>
          ...
          ))}</div> : <EmptyState title="Belum ada data pelanggan">Hubungi administrator untuk menghubungkan data pelanggan ke akun Anda.</EmptyState>}
        </section>
      )}
